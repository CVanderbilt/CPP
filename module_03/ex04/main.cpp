...
void ft_show(const Supertrap& f){
    std::cout << " ->Supertrap: " << f.getName() << "(" << f.getLevel() << ")" << std::endl;
    std::cout << " * hp & max hp: " << f.getHP() << ", " << f.getMaxHP() << std::endl;
    std::cout << " * enery & max energy: " << f.getEnergy() << ", " << f.getMaxEnergy() << std::endl;
    std::cout << " * melee, ranged & armor: " << f.getMeleeDamage() << ", " << f.getRangedDamage() << ", " << f.getArmor() << std::endl;
}
...
int main(void)
{
    ...
    Supertrap s1;
    Supertrap s2("claptrap");

    ft_show(s2);

    s2.ninjaShoebox(ninja2_cpy);
    s2.ninjaShoebox(robot_1);
    s2.ninjaShoebox(robot_3_cpy);

    s1.vaulthunter_dot_exe(":)");
    s1.vaulthunter_dot_exe(":)");
    s1.vaulthunter_dot_exe(":)");
    s1.vaulthunter_dot_exe(":)");
